...
function BidModal({ showBidModal,setShowBidModal,product,reloadData}) {
    ...
    const onFinish=async(values)=>{
        try {
            ...
            if (response.success){
                message.success("Bids Added succesfully");
                reloadData();
   ...
   onOk={()=>formRef.current.submit()}>
    <div className='flex flex-col gap-5 mb-5'>
        ...
        >
            ...
            name='bidAmount' rules={rules}><Input/></Form.Item>
             <Form.Item label="Message"
            name='message' rules={rules}><Input.TextArea/></Form.Item>
             <Form.Item label="Mobile"
            name='mobile' rules={rules}><Input type='number'/></Form.Item>
        </Form>
    </div>
   </Modal>
  )
